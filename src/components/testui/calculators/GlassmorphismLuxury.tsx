...
export function GlassmorphismLuxury() {
  ...
  return (
    <div className="h-screen w-full relative overflow-hidden flex" style={{ backgroundColor: '#F8F4F0' }}>
      ...
      <div className="absolute top-32 left-1/2 transform -translate-x-1/2 flex gap-4 z-20">
        {[1, 2, 3].map((i) => (
          <motion.div
            key={i}
            className={`w-12 h-12 rounded-full border-2 flex items-center justify-center ${
              step >= i 
                ? 'bg-white/20 border-[#2F4A3C] text-[#2F4A3C]' 
                : 'border-[#2F4A3C]/30 text-[#2F4A3C]/50'
            }`}
      ...
      <AnimatePresence mode="wait">
        ...
        {step === 1 && (
          ...
          >
            {/* Calculator - Centered */}
            <div 
              className="w-2/5 max-w-lg p-12 rounded-3xl text-center"
              style={{
                background: 'linear-gradient(90deg, #3A5647 0%, #2F4A3C 50%, #2F4A3C 100%)',
            ...
            >
              <h1 className="font-serif mb-4">
                <span className="text-4xl text-[#F8F4F0]">Estimate Your Villa's</span>
                <br />
                <span className="text-3xl text-[#C96F4A]">Earning Potential</span>
              </h1>
              <p className="text-lg text-[#F8F4F0]/80 mb-8 font-light">
              ...
              <div className="space-y-4">
                {locations.map((loc) => (
                  ...
                  >
                    <div className="text-2xl font-serif text-[#F8F4F0]">{loc}</div>
                    <div className="text-sm text-[#F8F4F0]/60 mt-1">Premium Location</div>
                  </motion.button>
                ))}
              </div>
            </div>
        ...
        {step === 2 && (
          ...
          >
            {/* Calculator - Centered */}
            <div 
              className="w-2/5 max-w-lg p-10 rounded-3xl"
              style={{
                background: 'linear-gradient(90deg, #3A5647 0%, #2F4A3C 50%, #2F4A3C 100%)',
                backdropFilter: 'blur(20px)',
                border: '1px solid rgba(255, 255, 255, 0.2)',
            ...
            >
              <h2 className="text-3xl font-serif text-[#F8F4F0] mb-2 text-center">
                Property Details
              </h2>
              <p className="text-base text-[#F8F4F0]/80 mb-6 text-center font-light">
              ...
              <div className="space-y-6">
                {/* Bedrooms */}
                <div>
                  <label className="block text-sm font-light text-[#F8F4F0]/80 mb-3">
                    BEDROOMS
                  </label>
                      ...
                      <button
                        ...
                        className={`flex-1 py-3 px-4 rounded-xl transition-all ${
                          bedrooms === (typeof num === 'string' ? 6 : num)
                            ? 'bg-[#C96F4A]/30 text-[#F8F4F0] border border-[#C96F4A]'
                            : 'bg-white/10 text-[#F8F4F0]/70 hover:bg-white/20'
                        }`}
                        style={{ backdropFilter: 'blur(10px)' }}
                      >
                        {num}
                ...
                >
                  <span className="text-[#F8F4F0]">Calculate Premium Returns</span>
                </motion.button>
              </div>
            </div>
          </motion.div>
          ...
          >
            ...
            >
              ...
              <div className="grid grid-cols-3 gap-4 mb-8">
                ...
                >
                  <div className="text-xs text-[#F8F4F0]/60 mb-2 font-light">MONTHLY REVENUE</div>
                  <div className="text-2xl font-serif text-[#F8F4F0]">
                ...
                >
                  <div className="text-xs text-[#F8F4F0]/60 mb-2 font-light">OCCUPANCY RATE</div>
                  <div className="text-2xl font-serif text-[#F8F4F0]">
                ...
                >
                  <div className="text-xs text-[#F8F4F0]/60 mb-2 font-light">NIGHTLY RATE</div>
                  <div className="text-2xl font-serif text-[#F8F4F0]">
              ...
              >
                <div className="grid grid-cols-3 gap-8">
                  <div>
                    <div className="text-xs text-[#F8F4F0]/60 mb-1 font-light">ANNUAL EARNINGS</div>
                    <div className="text-xl font-serif text-[#F8F4F0]">
                      {formatCurrency(displayValues.annual)}
                    </div>
                  </div>
                  <div>
                    <div className="text-xs text-[#F8F4F0]/60 mb-1 font-light">MANAGEMENT FEE</div>
                    <div className="text-xl font-serif text-[#F8F4F0]/80">
                      -{formatCurrency(results.managementFee)}
                    </div>
                  </div>
                  <div>
                    <div className="text-xs text-[#F8F4F0]/60 mb-1 font-light">NET INCOME</div>
                    <div className="text-2xl font-serif text-[#F8F4F0]">
